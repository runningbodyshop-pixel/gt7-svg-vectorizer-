...
st.set_page_config(page_title="Anime SVG Vectorizer 1.6", layout="wide")
...
def build_generic_anime_palette(
    ...
    if keep_neutrals:
        ...
        # light neutral
        light_mask = (lum >= np.percentile(lum, 84)) & (sat <= 0.30)
    ...
    if light_tone_count > 0:
        light_mask = (
            (lum >= np.percentile(lum, 72))
            & (sat <= 0.38)
        )
        light_pixels = pixels[light_mask]
        light_pairs = quantize_palette_from_pixels(light_pixels, light_tone_count)
        anchors.extend([c for c, count in light_pairs])

    if skin_tone_count > 0:
        skin_mask = detect_skin_pixels(pixels)
        skin_pixels = pixels[skin_mask]
        skin_pairs = quantize_palette_from_pixels(skin_pixels, skin_tone_count)
...
def make_lineart_mask(
    ...
    edge_like = (local_contrast >= contrast_threshold) | (edge >= contrast_threshold * 1.25)

    if mode == "エッジ優先・汎用":
        mask = fg & dark & edge_like
    elif mode == "黒インク強め":
        very_dark = lum <= max(20, dark_threshold * 0.72)
        mask = fg & ((dark & edge_like) | (very_dark & (local_contrast >= contrast_threshold * 0.55)))
    else:
        soft_dark = lum <= min(175, dark_threshold + 35)
        low_sat_shadow = sat <= 0.40
    ...
    # ノイズ除去
    if remove_tiny_px > 0:
        size = remove_tiny_px * 2 + 1
        mask_img = mask_img.filter(ImageFilter.MinFilter(size))
        mask_img = mask_img.filter(ImageFilter.MaxFilter(size))

    # 太さ調整（負なら細く、正なら太く）
    if line_width_adjust > 0:
        size = line_width_adjust * 2 + 1
        mask_img = mask_img.filter(ImageFilter.MaxFilter(size))
    elif line_width_adjust < 0:
        size = abs(line_width_adjust) * 2 + 1
        mask_img = mask_img.filter(ImageFilter.MinFilter(size))
...
def make_lineart_group(
    ...
    mask = make_lineart_mask(
        ...
        line_width_adjust=line_width_adjust,
    ...
    if line_color_mode == "自動":
        fill = estimate_dark_color(rgba_img, mask_bool, fallback="#111318")
    else:
        fill = manual_color
...
def run_potrace_color_trace(
    ...
    if add_lineart:
        line_group, line_mask_img = make_lineart_group(
            rgba_img=img,
            alpha_threshold=alpha_threshold,
            mode=line_mode,
            ...
            smooth_corners=max(0.35, min(smooth_corners, 1.10)),
            optimize=min(optimize, 0.30),
            remove_tiny_px=line_remove_tiny_px,
        )
        if line_mask_img is not None:
            line_mask_bool = np.array(line_mask_img) > 0
    ...
    for idx in order:
        ...
        if cut_line_from_fills and line_mask_bool is not None:
            mask_bool = mask_bool & (~line_mask_bool)
    ...
    return svg, line_mask_img, palette
...
st.title("Anime SVG Vectorizer 1.6")
st.caption(
    "白フチ対策 + 汎用アニメ絵向けパレット補正 + 線画レイヤー調整強化版。"
    "線画を細く/太く、肌色・薄い色を保護、色数増加に対応。"
)
...
with st.sidebar:
    ...
    alpha_threshold = st.slider(
        ...
        254,
        58,
        help="外周の白/灰色フチが出るなら上げる。欠けるなら下げる。",
    )

    harden_alpha = st.checkbox(
        "半透明の縁を締める",
        value=True,
        help="輪郭の半透明縁を不透明化して白っぽいにじみを減らします。",
    )
    ...
    color_layer_overlap = st.slider(
        ...
        3,
        1,
        help="色面境界のスキマを減らす。太りすぎるなら0。",
    )

    st.subheader("色パレット")

    fallback_colors = st.slider(
        "総色数",
        2,
        48,
        18,
        help="増やすほど元絵に近づくが、SVGが重くなりやすい。",
    )

    use_priority_palette = st.checkbox("優先色保護を使う", value=True)

    keep_neutrals = st.checkbox(
        "黒/白/グレー系を保護",
        value=True,
        help="線画、白服、グレー影を残しやすくします。",
    )

    accent_count = st.slider(
        "高彩度アクセント色の保護数",
        0,
        8,
        3,
        help="青、赤、ピンク、黄などの強い差し色を残しやすくします。",
    )

    light_tone_count = st.slider(
        "明るい薄色の保護数",
        0,
        8,
        3,
        help="白服、薄い髪、明るいハイライトなどを残しやすくします。",
    )

    skin_tone_count = st.slider(
        "肌色保護数",
        0,
        8,
        2,
        help="肌の薄い色の階調を残しやすくします。",
    )
    ...
    palette_merge_distance = st.slider(
        ...
        80,
        22,
        help="小さいほど色を分けやすい。大きいほど似た色をまとめる。",
    )
    ...
    min_color_area_px = st.slider(
        ...
        200,
        8,
        help="小さいほど細部色を残しやすい。大きいほどノイズを減らせる。",
    )
    ...
    remove_largest_color = st.checkbox(
        "最大面積色を背景として削除",
        value=False,
        help="透明PNGなら通常OFFでOK。背景つきJPGで必要な時だけON。",
    )
    ...
    line_contrast_threshold = st.slider(
        "線画: コントラストしきい値",
        4,
        90,
        24,
        help="下げるほど多く拾う。上げるほどシャープになる。",
    )

    line_width_adjust = st.slider(
        "線画の太さ調整",
        -3,
        3,
        -1,
    ...
    line_remove_tiny_px = st.slider(
        "線画ノイズ除去",
        0,
        2,
        0,
        help="小さいゴミ線を減らす。強すぎると細線が消える。",
    )

    cut_line_from_fills = st.checkbox(
        "塗りレイヤーから線画部分を少し外す",
        value=True,
        help="ONにすると線画の二重化を減らし、太りすぎを抑えやすい。",
    )
    ...
    line_manual_color = st.color_picker("手動線画色", "#15171c")
        ...
        with tempfile.TemporaryDirectory() as td:
                ...
                try:
                    with st.spinner("Potraceでトレース中..."):
                        svg_text, debug_line_mask, debug_palette = run_potrace_color_trace(
                            ...
                            line_manual_color=line_manual_color,
                            line_remove_tiny_px=line_remove_tiny_px,
                            cut_line_from_fills=cut_line_from_fills,
                            speckles=speckles,
                            smooth_corners=smooth_corners,
                            optimize=optimize,
                        )
                        used = "Potrace + palette control + lineart control"
                except Exception as e:
                    errors.append(str(e))
                    st.error(str(e))
            ...
            if svg_text:
                ...
                with col2:
                    ...
                    if preview:
                        st.image(preview, caption="SVGプレビュー", use_container_width=True)
                    else:
                ...
                st.download_button(
                    "SVGをダウンロード",
                    data=svg_bytes,
                    file_name=make_download_name(uploaded.name, "anime_v16"),
                    mime="image/svg+xml",
                ...
                if show_debug:
                    with st.expander("デバッグ"):
                        ...
                        if debug_palette is not None and len(debug_palette) > 0:
                            swatch_h = 48
                            swatch_w = max(48, 48 * len(debug_palette))
                            swatch = Image.new("RGB", (swatch_w, swatch_h), (255, 255, 255))

                            for i, c in enumerate(debug_palette):
                                block = Image.new("RGB", (48, swatch_h), tuple(int(x) for x in c))

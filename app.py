...
# =========================================================
# GT7 / Forza-style Layered SVG Vectorizer V4
# Quality Budget Edition
# =========================================================

st.set_page_config(
    page_title="GT7 / Forza-style Layered SVG Vectorizer V4",
    layout="wide",
)
...
PRESETS: Dict[str, AppConfig] = {
    "Quality Balanced": AppConfig(
        ...
        svg_limit_kb=14.5,
        budget=RoleBudget(base=140, light=95, skin=80, dark=105, shadow=95, accent=80, outline=180, face=130),
    ),
    "Anime High Quality": AppConfig(
        ...
        svg_limit_kb=14.5,
        budget=RoleBudget(base=170, light=120, skin=110, dark=135, shadow=120, accent=110, outline=240, face=190),
    ),
    "Safe Universal": AppConfig(
        ...
        svg_limit_kb=14.5,
        budget=RoleBudget(base=105, light=70, skin=60, dark=75, shadow=65, accent=65, outline=125, face=95),
    ),
    "Line Art Focus": AppConfig(
        ...
        svg_limit_kb=14.5,
        budget=RoleBudget(base=90, light=65, skin=80, dark=85, shadow=70, accent=80, outline=260, face=220),
    ),
}
...
def extract_line_parts(
    ...
    if cfg.enable_boundary_lines:
        ...
        boundary = cv2.bitwise_and(boundary, cv2.bitwise_not(erode_mask(fg_mask, 1) == 0).astype(np.uint8) * 255)
...
def apply_quality_budget(parts: List[VectorPart], cfg: AppConfig, logs: List[str]) -> List[VectorPart]:
    ...
    max_total = sum(asdict(cfg.budget).values())
...
def generate_svg_files(parts: List[VectorPart], shape_hw: Tuple[int, int], cfg: AppConfig, logs: List[str]) -> Dict[str, str]:
    ...
    role_order = ["base", "light", "dark", "skin", "shadow", "accent", "outline_boundary", "outline_dark", "outline_face"]
...
def process_image_pipeline(img_rgb: np.ndarray, cfg: AppConfig) -> ProcessResult:
    ...
    skin_mask = build_skin_mask(processed_bgr, fg_mask, subject_mask, logs) if cfg.protect_skin else np.zeros_like(fg_mask)
    accent_mask = build_accent_mask(processed_bgr, fg_mask, logs) if cfg.protect_accents else np.zeros_like(fg_mask)
    light_subject_mask = build_light_subject_mask(processed_bgr, fg_mask, logs) if cfg.protect_light_subject else np.zeros_like(fg_mask)
    ...
    quant_bgr, labels, centers = quantize_foreground(processed_bgr, fg_mask, cfg.num_colors, logs)
    ...
    parts = apply_quality_budget(fill_parts + line_parts, cfg, logs)
...
st.title("GT7 / Forza-style Layered SVG Vectorizer V4")
st.caption("Quality Budget版：顔・線・濃色・白服/明色・肌・差し色を役割別予算で保護します。")
...
if uploaded is not None:
    ...
    if st.button("Generate SVGs", type="primary", use_container_width=True):
        try:
            with st.spinner("Processing image with quality budget pipeline..."):
...
if "last_result" in st.session_state:
    ...
    with tabs[5]:
        ...
        zip_name = f"{source_base}_layered_svg_bundle_v4.zip"
        st.download_button(

...
# =========================================================
# GT7 / Forza-style Layered SVG Vectorizer V5
# Color / Line / Alpha Separation Edition
# =========================================================

st.set_page_config(
    page_title="GT7 / Forza-style Layered SVG Vectorizer V5",
    layout="wide",
)
...
def open_uploaded_image(uploaded_file) -> Tuple[np.ndarray, np.ndarray]:
    ...
    V5 keeps alpha instead of immediately baking it away. This helps transparent
...
def build_line_suppression_mask(img_bgr: np.ndarray, fg_mask: np.ndarray, cfg: AppConfig, logs: List[str]) -> np.ndarray:
    """Mask dark/ink-like pixels to remove from color quantization source.

    The line source is still extracted from the original processed image. This mask
    is only for creating a cleaner color_source so fills are not contaminated by
    black outlines.
    """
    ...
    ink = clean_mask(ink, open_k=2, close_k=2)
    ink = dilate_mask(ink, cfg.line_mask_dilate)
    logs.append(f"Line suppression pixels={int(np.count_nonzero(ink))}")
    return ink


def build_color_source(img_bgr: np.ndarray, fg_mask: np.ndarray, line_mask: np.ndarray, cfg: AppConfig, logs: List[str]) -> np.ndarray:
    if not cfg.enable_line_inpaint or np.count_nonzero(line_mask) == 0:
        logs.append("Color source: using processed image without line inpaint")
        return img_bgr.copy()

    # Inpaint only inside foreground, then lightly smooth. This makes KMeans build
    # clean fill surfaces while preserving the original image for final line layers.
    mask = cv2.bitwise_and(line_mask, fg_mask)
    radius = max(1, int(cfg.line_inpaint_radius))
    try:
        color_source = cv2.inpaint(img_bgr, mask, radius, cv2.INPAINT_TELEA)
        color_source = cv2.bilateralFilter(color_source, d=5, sigmaColor=26, sigmaSpace=26)
        logs.append(f"Color source: line inpaint enabled radius={radius}")
        return color_source
    except Exception as e:
        logs.append(f"Color source: inpaint failed, fallback to processed image: {e}")
        return img_bgr.copy()
...
def build_light_subject_mask(img_bgr: np.ndarray, fg_mask: np.ndarray, logs: List[str]) -> np.ndarray:
    hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)
    sat = hsv[:, :, 1]
    val = hsv[:, :, 2]

    # Protect foreground-internal whites and light grays such as white clothing / hair / accessories.
    light = ((val > 180) & (sat < 70)).astype(np.uint8) * 255
    light = cv2.bitwise_and(light, erode_mask(fg_mask, 1))
    light = clean_mask(light, open_k=3, close_k=5)
    logs.append(f"Light-subject pixels={int(np.count_nonzero(light))}")
    return light
...
def extract_underpaint_parts(
    ...
    if not cfg.enable_underpaint or np.count_nonzero(fg_mask) == 0:
        return []

    # Large, smooth pieces below everything. This hides tiny seams without
    # replacing detailed fills. Use the foreground components rather than a
    # single giant rectangle so decals remain usable as parts.
    mask = clean_mask(fg_mask, open_k=5, close_k=13)
    mask = dilate_mask(mask, max(1, cfg.overlap_px + 1))
    parts: List[VectorPart] = []
...
def extract_fill_parts(
    ...
    for idx in cluster_indices:
        ...
        if cfg.overlap_px > 0 and role in ("base", "light", "skin"):
...
def apply_spatial_coverage(
    ...
    for cell_y in range(3):
        for cell_x in range(3):
            cell = (cell_x, cell_y)
            need = max(0, cfg.spatial_min_parts_per_cell - cell_counts.get(cell, 0))
            if need <= 0:
                continue
...
def process_image_pipeline(img_rgb: np.ndarray, alpha: np.ndarray, cfg: AppConfig) -> ProcessResult:
    ...
    light_subject_mask = build_light_subject_mask(color_source_bgr, fg_mask, logs) if cfg.protect_light_subject else np.zeros_like(fg_mask)
...
st.title("GT7 / Forza-style Layered SVG Vectorizer V5")
st.caption("V5：RGBA/alpha保持、色面用ソースと線画ソース分離、underpaint、spatial coverageを追加した高品質版です。")
...
with st.sidebar:
    ...
    with st.expander("Line settings"):
        ...
        line_inpaint_radius = st.slider("Color-source line inpaint radius", 1, 7, base_cfg.line_inpaint_radius, 1)
        line_mask_dilate = st.slider("Line suppression dilation", 0, 5, base_cfg.line_mask_dilate, 1)
...
if uploaded is not None:
    ...
    if st.button("Generate SVGs", type="primary", use_container_width=True):
        try:
            with st.spinner("Processing image with V5 color/line/alpha pipeline..."):
...
if "last_result" in st.session_state:
    ...
    tabs = st.tabs(["Previews", "Masks", "SVG Files", "Stats", "Logs / Debug", "Download"])
    ...
    with tabs[5]:
        ...
        zip_name = f"{source_base}_layered_svg_bundle_v5.zip"
        st.download_button(

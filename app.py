...
APP_TITLE = "GT7 SVG Paint Stack Edition Crashfix"
...
def detect_skin(bgr: np.ndarray, visible: np.ndarray, guides: dict) -> np.ndarray:
    ...
    n, labels, stats, _ = cv2.connectedComponentsWithStats((skin > 0).astype(np.uint8), connectivity=8)
...
st.caption("軽いベースを先に作り、あとから細部を積み上げる方式です。今回版はクラッシュ対策を優先し、実データSVGの巨大プレビューを控えめにしています。")
...
with st.sidebar:
    ...
    base['show_svg_preview'] = st.toggle("実SVGプレビューを表示（重い時はOFF）", False)
...
total_svg_bytes = sum(len(svg.encode('utf-8')) for _, svg in files)
if st.session_state.cfg.get('show_svg_preview', False) and total_svg_bytes < 650_000 and len(files) <= 40:
    render_side_by_side(res['work_image'], files, res['work_image'].width, res['work_image'].height)
else:
    st.subheader("軽量プレビュー")
    st.image(res['work_image'], caption="処理後の元画像", use_container_width=True)
    st.info("スマホ/Streamlit保護のため、実SVGの重ね表示は初期OFFです。ZIP内の preview.html で重ね合わせを確認できます。必要ならサイドバーでONにしてください。")

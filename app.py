...
def main() -> None:
    ...
    st.caption("大きい下地 → 差分ディテール → 影/ハイライト → 線画 の順で、荒れを減らしながらSVG化する v4 版です。")

    with st.expander("今回の“荒削り”への対策", expanded=True):
        st.markdown(
            """
...
- **toneレイヤーに軽いぼかしを入れて荒れを抑える**
    ...
    with st.expander("線画レイヤー設定", expanded=False):
        ...
        with l2:
            ...
            line_min_length = st.slider("Canny線の最小長さ", 4, 300, 18, 1)
            line_max_count = st.slider("Canny線の最大本数", 20, 1200, pv["line_max"], 10)
    ...
    if st.button("SVGに変換する", type="primary", use_container_width=True):
        with st.spinner("base → detail差分 → tone → line の順でSVG化しています…"):
    ...
    st.info(
        "近くで荒い場合のおすすめ: detail最小面積を上げる / baseとの差分しきい値を上げる / detail複雑度上限を下げる / toneぼかしカーネルを上げる / line最大本数を下げる"
    )
    ...
    with dl1:
        st.download_button(
            ...
            file_name="vectorized_svg_layers_v4.zip",

...
with st.sidebar:
    ...
    max_side = st.slider("処理前の最大辺px", 512, 2600, 1400, 64)
    ...
    edge_trim_px = st.slider(
        ...
        5,
        1,
        help="謎の外周線や白/灰色フチが出る時は1〜3。輪郭が欠ける時は0かマイナス。",
    )
    ...
    underpaint_expand = st.slider(
        ...
        6,
        0,
        help="プラスで外に広げる。謎の輪郭が出る時は0〜-2。隙間が出る時は1〜2。",
    )
    ...
    underpaint_manual_color = st.color_picker("手動下地色", "#0b0d10")
    ...
    color_layer_overlap = st.slider(
        ...
        3,
        0,
        help="プラスで隙間対策、マイナスで色のはみ出し/謎輪郭を抑える。",
    )

    fill_open_px = st.slider("塗りノイズ除去px", 0, 2, 0)

    fill_close_px = st.slider("塗り小穴埋めpx", 0, 2, 0)

    st.subheader("色パレット")

    colors = st.slider("総色数", 2, 64, 26)
    ...
    palette_merge_distance = st.slider(
        ...
        80,
        14,
        help="小さいほど肌や薄い色が分かれやすい。大きいほど軽くなる。",
    )

    min_color_area_px = st.slider("小さい色領域を捨てる基準px", 1, 250, 4)

    resample_color = st.checkbox("元画像から塗り色を取り直す", value=True)
    ...
    vibrance = st.slider("色の鮮やかさ補正", -30, 50, 8)
    ...
    with st.expander("手動保護色", expanded=False):
        ...
        manual_color_2 = st.color_picker("手動色2", "#f0ded5")
    ...
    main_line_dark_threshold = st.slider("主線: 暗さしきい値", 20, 180, 92)

    main_line_contrast_threshold = st.slider("主線: コントラストしきい値", 4, 100, 32)

    main_line_width_adjust = st.slider("主線の太さ調整", -4, 3, -2)

    main_line_open_px = st.slider("主線ノイズ除去px", 0, 2, 0)

    main_line_close_px = st.slider("主線の切れ補修px", 0, 2, 0)

    main_line_max_lum = st.slider("主線: 最大明るさ", 60, 220, 150)
    ...
    add_detail_line = st.checkbox("細部線を別レイヤーで追加", value=True)

    detail_line_dark_threshold = st.slider("細部線: 暗さしきい値", 20, 220, 145)

    detail_line_contrast_threshold = st.slider("細部線: コントラストしきい値", 4, 100, 42)

    detail_line_width_adjust = st.slider("細部線の太さ調整", -3, 2, -1)

    detail_line_open_px = st.slider("細部線ノイズ除去px", 0, 2, 0)

    detail_line_close_px = st.slider("細部線の切れ補修px", 0, 2, 0)

    detail_line_max_lum = st.slider("細部線: 最大明るさ", 80, 240, 185)
    ...
    detail_line_manual_color = st.color_picker("手動細部線色", "#39414a")
    ...
    cut_line_from_fills_px = st.slider(
        ...
        4,
        1,
        help="線が二重に太る時は1〜2。色面が欠ける時は0。",
    )

    st.subheader("Potrace詳細")

    fill_speckles = st.slider("塗り: 小ゴミ除去", 0, 100, 1)

    fill_smooth_corners = st.slider("塗り: 角の滑らかさ", 0.0, 2.0, 0.75, 0.05)

    fill_optimize = st.slider("塗り: パス最適化", 0.0, 5.0, 0.15, 0.05)

    line_speckles = st.slider("線: 小ゴミ除去", 0, 50, 0)

    line_smooth_corners = st.slider("線: 角の滑らかさ", 0.0, 2.0, 0.55, 0.05)

    line_optimize = st.slider("線: パス最適化", 0.0, 2.0, 0.08, 0.02)

    under_speckles = st.slider("下地: 小ゴミ除去", 0, 100, 2)

    under_smooth_corners = st.slider("下地: 角の滑らかさ", 0.0, 2.0, 1.0, 0.05)

    under_optimize = st.slider("下地: パス最適化", 0.0, 5.0, 0.25, 0.05)
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                st.download_button(
                    "SVGをダウンロード",
                    data=svg_bytes,
                    file_name=make_download_name(uploaded.name, "anime_v17_tuned"),
                    mime="image/svg+xml",
                    use_container_width=True,
                )
                ...
                if kb > 15:
                    st.warning(
                        "GT7の15KB制限には大きいです。"
                        "この品質調整版で見た目を詰めた後、次に分割出力へ進めるのがおすすめです。"
                    )
